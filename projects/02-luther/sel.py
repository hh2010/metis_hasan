""" Run Selenium separately to import from web """

import pandas as pd
import _pickle as pickle
import time
from tqdm import tqdm
from data import yah
...
for t in tqdm(ticks):
    try:
        ...
        cash.append(yaht[0])
        debt.append(yaht[1])
        ebitda.append(yaht[2])
        beta.append(yaht[3])
        inside.append(yaht[4])
        inst.append(yaht[5])
        ticky.append(t)
    except:
        continue

df = pd.DataFrame({'tick': ticky, 'cash': cash, 'debt': debt, 'ebitda': ebitda,
                   'beta': beta, 'inside': inside, 'inst': inst})

df.to_pickle('yah.pkl')
